...
def get_engineering_high_salary_employees():
   query = collection.find(
        {'job_role.title': 'Engineer', 'salary': {'$gt': 65000}}
...
def get_top_seniority_employees_excluding_hr():
    query = collection.find(
        {'department':{'$ne':'HR'}},{'_id':0}).sort('years_at_company',-1).limit(7)
...
def get_managers_excluding_departments():
    query = collection.find(
        ...
        'job_role.department':{'$nin':['Sales','Marketing']},},{'_id':0})
